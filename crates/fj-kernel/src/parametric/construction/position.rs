...
/// A position that can be losslessly converted into its canonical form
...
impl<const D: usize> Add<Vector<D>> for Position<D> {
...
impl<const D: usize> Sub<Self> for Position<D> {
...
impl<const D: usize> Sub<fj_math::Point<D>> for Position<D> {
    type Output = Vector<D>;
