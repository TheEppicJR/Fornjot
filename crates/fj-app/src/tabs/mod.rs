//! taken from https://github.com/Adanos020/egui_dock licenced as MIT
//! Intend to use this until it becomes a package and maybe make it nativly work with bevy
        ...
        match &mut tree[tree_index] {
            ...
            Node::Leaf {
                        ...
                        for (tab_index, tab) in tabs.iter().enumerate() {
                            ...
                            if is_being_dragged {
                                    ...
                                    ui.ctx().pointer_interact_pos()
                                {
                                    ...
                                    let start =
                                        state.drag_start.unwrap_or(center);
                                    ...
                                    {
                                        ui.ctx()
                                            .translate_layer(layer_id, delta);

                                        drag_data =
                                            Some((tree_index, tab_index));
                                    }
                                }
                                ...
                                let response =
                                    ui.interact(response.rect, id, sense);
                ...
                if is_being_dragged && full_response.hovered() {
                    hover_data =
                        ui.input().pointer.hover_pos().map(|pointer| {
                            HoverData {
                                rect,
                                dst: tree_index,
                                tabs: tabs_response
                                    .hovered()
                                    .then(|| tabs_response.rect),
                                pointer,
                            }
                        });
                }
