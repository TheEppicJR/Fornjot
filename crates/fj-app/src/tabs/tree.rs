...
impl<Context> Tree<Context> {
    ...
    pub fn append_tab_to_tabs(&mut self, tab: Tab<Context>) {
        let mut active_tab = self.find_active();
        match active_tab {
            None => {
                self.tree[NodeIndex.root()].append_tab(tab);
            }
            Some(cur_tab) => {
                cur_tab.append_tab(tab);
            }
        }
    }

    /// Returns the viewport `Rect` and the `Tab` inside the first leaf node, or `None` of no leaf exists in the `Tree`.
    pub fn find_active<T: Tab<Context> + 'static>(
        &mut self,
    ) -> Option<(Rect, &mut T)> {
    ...
    pub fn split(
        ...
        {
            let index =
                self.tree.iter().rposition(|n| !n.is_none()).unwrap_or(0);
        ...
        if node.is_left() {
            'left_end: loop {
                ...
                for (dst, src) in dst.zip(src) {
                    ...
                    self.tree[dst] =
                        std::mem::replace(&mut self.tree[src], Node::None);
                }
                level += 1;
            }
        } else {
